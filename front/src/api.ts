...
export async function authorisedRequest<T=any>(url: string, options: ApiRequestOptions = {}) {
  ...
  if (res.status === 401 && localStorage.getItem("refreshToken")) { // session.refreshToken){
...
export async function refreshAccessToken(): Promise<boolean> {
  const refreshToken =  localStorage.getItem("refreshToken");//session.refreshToken
  ...
  if (data.refreshToken) localStorage.setItem("refreshToken", data.refreshToken);//session.refreshToken = data.refreshToken 
