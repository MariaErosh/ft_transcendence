...
import { matchDuration, matchResults, activeMatches } from "../../../../observability/dist/metrics/metrics"; // Import metrics
...
export function updatePos(): void | number {
    ...
    if (gameState.ball.x - board.BALL_RADIUS / 2 <= 0 || gameState.ball.x + board.BALL_RADIUS / 2 >= board.CANVAS_WIDTH) {
        ...
        logger.info("Score updated", {
            matchId: gameState.current.matchId,
        ...
        if (gameState.score.right >= 5 || gameState.score.left >= 5) {
            ...
            logger.info("Match finished", {
                matchId: gameState.current.matchId,
                winner: gameState.winner.alias,
                loser: gameState.loser.alias,
                finalScore: `${gameState.score.left}:${gameState.score.right}`
            });

            matchResults.labels(rightWins ? "win" : "loss", gameState.current.type, process.env.SERVICE_NAME).inc();
            activeMatches.labels(gameState.current.type, process.env.SERVICE_NAME).set(0);
...
export function resetSpecs(next: GameObject | -1) {
    ...
    if (next === -1 || next.matchId === -1) {
        ...
        logger.info("No next game available");
        ...
        activeMatches.labels("none", process.env.SERVICE_NAME).set(0);
    } else {
        gameState.current = next;
        logger.info("Game specs reset for new match", {
            matchId: next.matchId,
            type: next.type,
            leftPlayer: next.leftPlayer.alias,
            rightPlayer: next.rightPlayer.alias
        });
        activeMatches.labels(next.type, process.env.SERVICE_NAME).set(1);
    }
